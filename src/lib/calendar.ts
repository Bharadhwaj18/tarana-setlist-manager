...
/**
 * Groups shows (by show_date), tasks (by due_date) and unavailability
 * (expanded across its whole start_date..end_date range) into one
 * YYYY-MM-DD-keyed lookup, so rendering a day cell is an O(1) lookup
 * instead of filtering every list per cell.
 */
export function groupItemsByDate(shows: Show[], tasks: Note[], unavailability: Unavailability[]): Record<string, DayItems> {
  const byDate: Record<string, DayItems> = {}
  const bucket = (date: string) => (byDate[date] ??= { shows: [], tasks: [], unavailability: [] })
